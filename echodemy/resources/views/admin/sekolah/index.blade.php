<x-app-layout>
    ...
    <!-- Search + Filter -->
    <form method="GET" action="{{ route('admin.sekolah.index') }}" id="filter-form" class="flex flex-wrap items-center gap-3 mb-5">
        ...
        <div class="relative flex-1 min-w-[240px]">
            ...
            <input type="text" name="search" id="search-input" value="{{ $search }}" placeholder="Cari Nama Sekolah, NPSN atau Kota..."
                ...
                @forelse ($sekolahs as $sekolah)
                    @php
                        ...
                        [$badgeLabel, $badgeClass] = match (true) {
                            ! $sekolah->is_active => ['Nonaktif', 'bg-coral/20 text-coral'],
                            $sekolah->status === \App\Models\Sekolah::STATUS_PENDING => ['Menunggu', 'bg-amber/20 text-amber'],
                            $sekolah->status === \App\Models\Sekolah::STATUS_VERIFIED => ['Aktif', 'bg-periwinkle/20 text-periwinkle'],
                            default => ['Ditolak', 'bg-coral/20 text-coral'],
                        };
                    @endphp
                    <tr class="border-b border-border last:border-0">
                        ...
                        <td class="px-5 py-4">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-medium {{ $badgeClass }}">{{ $badgeLabel }}</span>
                        </td>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <div class="relative group">
                                    <button type="button" class="view-btn w-8 h-8 rounded-lg bg-teal/20 text-teal flex items-center justify-center hover:bg-teal/30" data-id="{{ $sekolah->id }}">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        </svg>
                                    </button>
                                    <span class="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-ink text-cream text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition">Lihat</span>
                                </div>

                                <div class="relative group">
                                    <button type="button" class="edit-btn w-8 h-8 rounded-lg bg-amber/20 text-amber flex items-center justify-center hover:bg-amber/30" data-id="{{ $sekolah->id }}">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <div class="relative group">
                                    ...
                                    <span class="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-ink text-cream text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition">Nonaktifkan</span>
                                </div>
                            </div>
                        </td>
                    </tr>
    ...
    <div id="view-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div class="relative bg-putih rounded-2xl shadow-lg w-full max-w-md p-6">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between"><span class="text-hitam2">NPSN</span><span id="view-npsn" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Jenjang</span><span id="view-jenjang" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Singkatan</span><span id="view-singkatan" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Kota/Kabupaten</span><span id="view-kota" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Alamat</span><span id="view-alamat" class="font-medium text-right max-w-[60%]">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Telepon</span><span id="view-telepon" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Email</span><span id="view-email" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Siswa/Guru</span><span id="view-siswa-guru" class="font-medium">-</span></div>
            </div>

            <button type="button" class="modal-close-btn w-full mt-6 bg-cream2 text-ink font-medium text-sm rounded-lg py-2.5 hover:bg-border">
                Tutup
            </button>
        </div>
    </div>
