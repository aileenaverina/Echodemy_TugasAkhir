<x-app-layout>
        ...
        @forelse ($pendingSekolahs as $sekolah)
            <div class="bg-white rounded-xl shadow-sm p-5 flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <button type="button" class="reject-open-btn border border-coral text-coral text-sm font-medium rounded-lg px-4 py-2 hover:bg-coral/5"
                        data-target="reject-modal-{{ $sekolah->id }}">
                        Tolak
                    </button>
                </div>
            </div>
            ...
            <div id="reject-modal-{{ $sekolah->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center px-4">
                <div class="reject-modal-overlay absolute inset-0 bg-ink/40"></div>
                <div class="relative bg-white rounded-2xl shadow-lg w-full max-w-md p-6">
                    ...
                    <form method="POST" action="{{ route('admin.sekolah-verification.reject', $sekolah) }}">
                        ...
                        <div class="flex justify-end gap-2 mt-5">
                            <button type="button" class="reject-cancel-btn text-sm font-medium text-ink/60 px-4 py-2 hover:text-ink">
                                Batal
                            </button>
                            <button type="submit"
    ...
    <script>
    $(function () {
        $('.reject-open-btn').on('click', function () {
            const target = $(this).data('target');
            $('#' + target).removeClass('hidden');
        });

        $('.reject-cancel-btn, .reject-modal-overlay').on('click', function () {
            $(this).closest('[id^="reject-modal-"]').addClass('hidden');
        });
    });
    </script>
</x-app-layout>