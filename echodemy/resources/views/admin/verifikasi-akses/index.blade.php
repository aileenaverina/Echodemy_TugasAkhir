<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs font-semibold tracking-widest text-coral uppercase mb-1">Verifikasi dan Akses</p>
    ...
    <!-- Akun Admin -->
    <h2 class="font-display text-lg font-semibold mb-3">Akun Administrator Platform</h2>
            ...
            <tbody>
                @forelse ($admins as $admin)
                    <tr class="border-b border-border last:border-0">
                        ...
                        <td class="px-5 py-4 text-hitam2">{{ $admin->last_login_at?->format('d/m/y H.i.s') ?? '-' }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <div class="relative group">
                                    <button type="button" class="view-btn w-8 h-8 rounded-lg bg-teal/60 text-teal flex items-center justify-center hover:bg-teal/30" data-id="{{ $admin->id }}">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <div class="relative group">
                                    <button type="button" class="delete-btn w-8 h-8 rounded-lg bg-coral/60 text-coral flex items-center justify-center hover:bg-coral/30"
                                        data-id="{{ $admin->id }}" data-nama="{{ $admin->nama_lengkap ?? $admin->email }}">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- Riwayat Keputusan -->
    <h2 class="font-display text-lg font-semibold mb-3">Riwayat Keputusan Verifikasi Sekolah</h2>
                ...
                @forelse ($riwayat as $log)
                    @php
                        [$label, $badgeClass] = match (true) {
                            str_starts_with($log->deskripsi, 'Memverifikasi') => ['Diverifikasi', 'bg-teal/60 text-teal'],
                            str_starts_with($log->deskripsi, 'Menolak') => ['Ditolak', 'bg-coral/60 text-coral'],
                            str_starts_with($log->deskripsi, 'Menonaktifkan') => ['Dinonaktifkan', 'bg-amber/60 text-amber'],
                    ...
                    <tr class="border-b border-border last:border-0">
                        ...
                        <td class="px-5 py-4">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-medium {{ $badgeClass }}">{{ $label }}</span>
                        </td>
    ...
    <div id="add-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div class="relative bg-putih rounded-2xl shadow-lg w-full max-w-md p-6">
            <p class="text-xs font-semibold tracking-widest text-coral uppercase mb-1">Akun Baru</p>
    ...
    <div id="view-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div class="relative bg-putih rounded-2xl shadow-lg w-full max-w-md p-6">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between"><span class="text-hitam2">Kode User</span><span id="view-kode" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Email</span><span id="view-email" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Terdaftar</span><span id="view-created" class="font-medium">-</span></div>
                <div class="flex justify-between"><span class="text-hitam2">Aktif Terakhir</span><span id="view-login" class="font-medium">-</span></div>
            </div>
